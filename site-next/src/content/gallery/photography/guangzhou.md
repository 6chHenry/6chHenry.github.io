---
title: "广州"
description: "珠江、骑楼、老街与现代塔楼并置，广州的城市气质总在烟火气和天际线之间切换。"
date: "2026-05-25"
updatedAt: "2026-05-25T11:34:31.310Z"
tags:
  - "photography"
  - "greater-bay-area"
  - "guangzhou"
draft: false
legacyPath: "/gallery/photography/guangzhou/"
cover: "/assets/gallery/photography/guangzhou.assets/Yat-sen_Mausoleum_front.JPG"
coverDesc: "中山纪念堂的正面立面，是广州近现代城市记忆中庄重的一幕。"
images:
  - "/assets/gallery/photography/guangzhou.assets/Yat-sen_Mausoleum_side.JPG"
  - "/assets/gallery/photography/guangzhou.assets/shamian_island.JPG"
imageDescs:
  - "从侧面看中山纪念堂，屋檐、台阶与树影让建筑线条更有层次。"
  - "沙面的街道、树影与欧陆立面，保留着广州安静、缓慢的一面。"
category: "photography"
---
## 广州 · Guangzhou

粤港澳三城游的起点。广州有珠江两岸的现代夜景，也有骑楼、茶楼与老街留下的生活纹理；它不是用单一地标定义的城市，而是在烟火气与都市感之间来回切换。
