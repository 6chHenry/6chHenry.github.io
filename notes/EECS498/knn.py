...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    dists = dists.to(dtype=x_train.dtype)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    # Replace "pass" statement with your code
    x_train_flat = x_train.view(num_train,-1)
    x_test_flat = x_test.view(num_train,-1)
    for i in range(num_train):
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_train_flat = x_train.view(num_train,-1)
    x_test_flat = x_test.view(num_test,-1)
    A = torch.sum(x_train_flat ** 2,dim = 1)
    B = torch.sum(x_test_flat ** 2,dim = 1)
    dists = A.view(-1,1) + B.view(1,-1) - 2 * torch.mm(x_train_flat,x_test_flat.t())
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    for i in range(num_test):
        ...
        unique_labels , counts = nearest_labels.unique(return_counts=True)
...
class KnnClassifier:
    ...
    def predict(self, x_test: torch.Tensor, k: int = 1):
        ...
        ######################################################################
        # Replace "pass" statement with your code
        y_test_pred = predict_labels(compute_distances_no_loops(self.x_train,x_test),self.y_train,k)
        ######################################################################
        #                         END OF YOUR CODE                           #
        ######################################################################
        return y_test_pred
...
def knn_cross_validate(
    ...
    for k in k_choices:
        ...
        for i in range(num_folds):
            ...
            x_train_combined = torch.cat([x_train_folds[j] for j in range(num_folds) if j != i],dim = 0)
            y_train_combined = torch.cat([y_train_folds[j] for j in range(num_folds) if j != i],dim = 0)
    ##########################################################################
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    best_k = 0
    k_avg = {}
    for k,acc_list in k_to_accuracies.items():
        avg = sum(acc_list) / len(acc_list)
        k_avg[k] = avg
    if k_avg:
        max_avg = max(k_avg.values())
        best_candidates = [k for k,avg in k_avg.items() if avg == max_avg]
